...
import com.email.springEmail.registration.token.TokenService;
...
public class RegistrationService {
  ...
  public String registration(Registration registration) throws Exception {
	  Boolean valid =validator.test(registration.getEmail());
	  if(!valid) {
		 throw new Exception("email not valid");
	  }
	  String token =
	 userService.signIn(new User(
	  ...
	  emailSending.emailSending(registration.getEmail(), buildEmail(registration.getName() ,link));
	  return token;
  }
  
  @Transactional
  public String confirmToken(String token) {
      Token confirmationToken = service
              .getToken(token)
              .orElseThrow(() ->
